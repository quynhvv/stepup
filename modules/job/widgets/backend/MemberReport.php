...
class MemberReport extends DiyWidget
{
    ...
    private function buildData()
    {
        // build data default
        $data = [
            'collection' => UserJobStats::getCollection()->name,
            'format' => $this->formatDate,
            'chartReport' => UserJobStats::find()->where([
                'date_time' => [
        ...
        foreach ($data['chartReport'] as $day_report) {
            ...
            foreach ($day_report->data as $role => $value) {
                // Build ykeys, labels and colors
                if (!in_array($role, $this->chartData['ykeys'])) {
                    $this->chartData['ykeys'][] = $role;
                    $this->chartData['labels'][] = $userJob->getRoleText($role);
                    ...
                    // Build query
                    if (isset($data['query']))
                        $data['query']['$group'][$role] = ['$sum' => '$data.' . $role];
                }
...
//            '_id',
        ...
        foreach ($this->overall_statistics as $role => $count) {
            array_push($columns, [
                'attribute' => 'data.' . $role,
                'header' => $userJob->getRoleText($role),
            ]);
        }
