...
class RbacController extends \app\components\BackendController {
...
//    }
    ...
    public function actionActionlist() {
        $assign = [];
        if (Yii::$app->request->get('generation') == '1') {
            AuthActionList::getActionsFromSource();
            return $this->redirect(['backend/rbac/actionlist']);
        } else {
            $queryParams = Yii::$app->request->getQueryParams();
            $searchModel = new AuthActionList;
    ...
    public function actionRole() {
        ...
        $treeHtml = '';
        $auth->createTreeHtml($tree, $treeHtml, ['class' => 'dd-list']);
        $assign['treeHtml'] = $treeHtml;

        Yii::$app->view->title = Yii::t($this->module->id, 'Role');
        Yii::$app->view->params['breadcrumbs'][] = ['label' => Yii::t($this->module->id, ucfirst($this->module->id)), 'url' => ['index']];
        Yii::$app->view->params['breadcrumbs'][] = Yii::$app->view->title;
    ...
    public function actionAssign($user_id) {
        ...
        $auth->createTreeAssignHtml($tree, $treeHtml, $assignments, ['class' => 'dd-list']);
        $assign['treeHtml'] = $treeHtml;
