...
namespace letyii\gii\generators\letyiimodel;
...
//    public $generateLabelsFromComments = false;
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [['db', 'ns', 'tableName', 'moduleName', 'modelClass', 'baseClass'], 'filter', 'filter' => 'trim'],
...
//            'generateLabelsFromComments' => 'Generate Labels DB Comments',
//        ]);
//    }
...
//        var_dump($relations);
        ...
        $this->baseClass = 'app\components\ActiveRecord';
        $this->ns = 'app\modules\\' . $this->moduleName . '\models';
        $this->messageCategory = $this->moduleName;

        foreach ($this->getTableNames() as $tableName) {
            $className = $this->generateClassName($tableName);
            $classNameBase = 'Base' . $className;
            $tableSchema = $db->getTableSchema($tableName);
            $tableSchemaString = $this->generateTableSchemaString($tableSchema);
            $tableName = '{{%' . str_replace($db->tablePrefix, '', $this->tableName) . '}}';
            ...
            $params = [
                ...
                'tableSchemaString' => $tableSchemaString,
                'labels' => $this->generateLabels($tableSchema),
                'rules' => $this->generateRules($tableSchema),
                'relations' => $relations,
            ];
            $files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $classNameBase . '.php',
                $this->render('modelBase.php', $params)
            );
            $file = Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $className . '.php';
            if (!file_exists($file)) {
                $files[] = new CodeFile(
                    $file,
...
//    }
    ...
    public function generateTableSchemaString($tableSchema) {
        $tableSchemaString = [];
        foreach ($tableSchema->columns as $schema)
            $tableSchemaString[] = $schema->name;

        return \yii\helpers\Json::encode($tableSchemaString);
    }
}
